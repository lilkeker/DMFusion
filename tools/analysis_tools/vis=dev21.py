...
def parse_args():
    ...
    parser.add_argument(
        '--root_path',
        type=str,
        default='/data0/yy_datasets/nuscenes_bevdet/',
        help='Path to nuScenes dataset')
    parser.add_argument(
        '--save_path',
        type=str,
        default='/data0/Bevdet_Fusion_Visualize/dmfusions_cp_visulaize',
...
color_map = {0: (255, 10, 0), 1: (0, 255, 10)}
...
def main():
    ...
    info_path = \
        '/data0/yy_datasets/nuscenes_bevdet/' + '/bevdetv2sweep10-nuscenes_infos_%s.pkl' % args.version
    ...
    for cnt, infos in enumerate(
        ...
        #                   dtype=np.uint8)
        canvas = np.ones((canva_size, canva_size, 3), dtype=np.uint8) * 255
        # draw lidar points
        ...
        # img[int(1800+(900 / scale_factor)):int(1800+(900 / scale_factor)) + canva_size, w_begin:w_begin + canva_size, :] = canvas
        img[900:900+ canva_size * scale_factor, w_begin:w_begin + canva_size, :] = canvas
