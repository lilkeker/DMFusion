...
def create_groundtruth_database(dataset_class_name,
    ...
    if dataset_class_name == 'KittiDataset':
        ...
        dataset_cfg.update(
            ...
            pipeline=[
                ...
                #     with_label_3d=True),
                dict(
                    type='LoadAnnotationsforDBSAMPLE',
                    classes=class_names,
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        input_dict = dataset.get_data_info(j)     #return  'gt_bboxes_3d' 'gt_labels_3d' 'gt_names'
        dataset.pre_pipeline(input_dict)
        example = dataset.pipeline(input_dict)
        annos = example['ann_info']
        image_idx = example['sample_idx']
        points = example['points'].tensor.numpy()     ######经过dataset.pipeline处理后points应该在ego 系
        # gt_boxes_3d = annos['gt_bboxes_3d'].tensor.numpy()  ####采用lidar下的gt
        gt_boxes_3d = example['gt_bboxes_3d'].tensor.numpy()  ####采用ego 系的gt
