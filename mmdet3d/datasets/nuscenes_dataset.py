...
from mmdet3d.core.visualizer.image_vis import draw_lidar_bbox3d_on_img,draw_depth_bbox3d_on_img,draw_camera_bbox3d_on_img
...
class NuScenesDataset(Custom3DDataset):
    ...
    CLASSES = ('car', 'truck', 'trailer', 'bus', 'construction_vehicle',
               ...
               'barrier')
    # CLASSES = [             ####### 在create_data_bevdet.py使用的class
    ...
    def get_data_info(self, index):
        ...
        # global to camego transform
        camego2global = np.eye(4, dtype=np.float32)
        camego2global[:3, :3] = Quaternion(
            info['cams']['CAM_FRONT']
            ['ego2global_rotation']).rotation_matrix
        camego2global[:3, 3] = info['cams']['CAM_FRONT'][
        ...
        ego2global = torch.from_numpy(ego2global)


        # lidar to ego transform
        lidar2lidarego = np.eye(4).astype(np.float32)
        ...
        lidar2camego = lidar2lidarego.matmul(ego2global.matmul(global2camego))

        input_dict["lidar2lidarego"] = lidar2lidarego
        input_dict["lidar2camego"] = lidar2camego
    ...
    def get_ann_info(self, index):
        ...
        #         count += 1
        #     else:
        #         assert count < len(gt_bboxes_3d), f"{count} is out of bounds for axis 0 with size{len(gt_bboxes_3d)}"
        #         if count < len(gt_bboxes_3d):
        #             gt_bboxes_3d = np.delete(gt_bboxes_3d, count, axis=0)
        ...
        #         gt_velocity = np.delete(gt_velocity, index, axis=0)
        #         gt_names_3d = np.delete(gt_names_3d, index, axis=0)
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            # trans = self.data_infos[sample_id]['ego2global_translation']
            # rot = self.data_infos[sample_id]['ego2global_rotation']
            ...
            for i, box in enumerate(boxes):
                ...
                nusc_box = NuScenesBox(center, wlh, quat, velocity=box_vel)   ##### 先转成nusc_box，bevfusion(mit)中的操作是调用output_to_nusc_box实现
                nusc_box.rotate(rot)
                nusc_box.translate(trans) ######在 nuscbox 中 再转成global 系，bevfusion(mit)的操作是调用lidar_nusc_box_to_global实现
    ...
    #             box_vel.append(0)
    #             quat = pyquaternion.Quaternion(axis=[0, 0, 1], radians=box_yaw)
    #             nusc_box = NuScenesBox(center, wlh, quat,
    #                                    velocity=box_vel)  ##### 先转成nusc_box，bevfusion(mit)中的操作是调用output_to_nusc_box实现
    #             nusc_box.rotate(rot)
    #             nusc_box.translate(trans)  ######在 nuscbox 中 再转成global 系，bevfusion(mit)的操作是调用lidar_nusc_box_to_global实现
    #             if np.sqrt(nusc_box.velocity[0] ** 2 +
    #                        nusc_box.velocity[1] ** 2) > 0.2:
    ...
    def show(self, results, out_dir, show=False, pipeline=None):
        ...
        for i, result in enumerate(results):
            ...
            if i == 10:
                exit()
    #
    # def show(self, results, out_dir, show=True, window_name="visual"):
    #     """Results visualization.
    #
    #     Args:
    #         results (list[dict]): List of bounding boxes results.
    ...
    #         points = Coord3DMode.convert_point(points, Coord3DMode.LIDAR,
    #                                            Coord3DMode.DEPTH)
    #         inds = result['pts_bbox']['scores_3d'] > 0.2
    ...
    #                                         Box3DMode.DEPTH)
    #         show_result(points, gt_bboxes, pred_bboxes, out_dir, file_name, show, window_name)
    #         # note:相机可视化
    #         if self.modality["use_camera"]:
    #             if 'img_bbox' in result.keys():
    #                 result = result['img_bbox']
    #             # 6张环视图
    #             for view_num in range(6):
    #                 img_path = example["img_metas"][0].data["filename"][view_num]
    #                 file_name = osp.split(img_path)[-1].split('.')[0]
    #                 # img, lidar2img = example["img"][0].data[view_num], example["img_metas"][0].data["lidar2img"][view_num]
    #                 lidar2img = example["img_metas"][0].data["lidar2img"][view_num]
    #                 img = cv2.imread(img_path)  # 这里读取的是原图，因此不需要lidar2img做改变
    #                 gt_bboxes = self.get_ann_info(i)['gt_bboxes_3d']
    #                 pred_bboxes = result["pts_bbox"]['boxes_3d'][inds]
...
def show_multi_modality_result(img,
        ...
        if show:
            ...
            mmcv.imshow(show_img, win_name=window_name, wait_time=0)
